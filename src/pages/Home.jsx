...
import { listarListas } from "../firebase/listas";
...
function Home() {
  ...
  useEffect(() => {
    console.log("Usuário na Home:", usuario);
  }, [usuario]);
          ...
          {listas.map((lista) => (
            <Card key={lista.id} className="mb-2">
              <Card.Body>
                ...
                <Button onClick={() => navigate(`/listas/editar/${item.id}`)}>Editar</Button>
                <Button variant="dark" as={Link} to={`/listas/${lista.id}`}>Ver Itens</Button>
              </Card.Body>
            </Card>
          ))}
        </section>
